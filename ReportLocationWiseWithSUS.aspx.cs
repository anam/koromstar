...
public partial class AgentWiseReport : System.Web.UI.Page
{
    ...
    private string ProcessXML(string xml, string FName, string MName, string LName)
    {
        ...
        try
        {
            ...
            string srchFirstName = FName.ToLower();
            string srchMiddleName = MName.ToLower();
            string srchLastName =LName.ToLower();
    ...
    protected void btnSUSCustomer_Click(object sender, EventArgs e)
    {
        string ids = "";

        foreach (GridViewRow item in gvLocation.Rows)
	    {
            GridView gvTRANS = (GridView)item.FindControl("gvTRANS");
            foreach (GridViewRow gr in gvTRANS.Rows)
            {
                CheckBox chkSelect = (CheckBox)gr.FindControl("chkSelect");
                
                if (chkSelect.Checked)
                {
                    HiddenField hfCustomerID = (HiddenField)gr.FindControl("hfCustomerID");

                    ids += hfCustomerID.Value + ",";
                }
            }
	    }


        CUSTOMERManager.UpdateSUS_CUSTOMER(ids.Substring(0,ids.Length-1),DateTime.Today.AddMonths(1).ToString("yyyy-MM-dd"));

        Response.Redirect("ReportLocationWiseWithSUS.aspx");
    }
}